...
canonical 转换默认不做自动响度归一化，也不把 gain 烘焙进母版。每个 Event 独立保存 `gain_db`，所有交互音进入统一 SFX bus，再乘用户的 Muted/Low/Medium/High。三声最坏并发由离线 peak simulation 验证；若可能 clipping，先调 Event gain，再考虑只衰减 canonical asset。不存在行业通用的 `-9 dBFS` 答案，初始 gain 必须由整套声音和 T-Embed 扬声器共同定标。
...
4. **引擎回归**：验证无 heap sample loop、三声并发不 clipping、Muted/StopAll、抢占、结束精确零、SDL dummy 与 firmware compile。
