...
当前 Semantic Hierarchy 共 17 项，另有 TimerComplete 业务 cue；按信息重要度控制密度，而不是让所有操作复用同一个滑音：

| 家族 | Cue | 设计职责 |
| --- | --- | --- |
| Input | Navigate、Boundary | 高频操作的低调触点；到边界必须与移动成功不同 |
| Action | Confirm、Back、ToggleOn、ToggleOff、Reject | 用方向、时序和材质区分进入/返回、状态开/关与未执行 |
| Surface | MenuOpen、MenuClose | 与菜单形变同帧的单动作上扬/下潜，不借用 Confirm/Back 双敲 |
...
| MenuOpen | Surface 从右侧牵开 | 低音区单起音上扬，音身短于动画 |
| MenuClose | Surface 向右卷收 | 同族单起音下潜，收尾比展开更干脆 |
...
该表保留为设计演进记录，不再是当前 palette 的实现约束。获批 Select 约 10 ms，Toggle 约 63–65 ms，MenuOpen/MenuClose 约 160/140 ms；当前 18 项枚举的精确参数与 golden 以 `sound_cue_library.cpp` 和测试为准，最终扬声器 gain 仍由真机决定。
