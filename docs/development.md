...
SIGHT 首次进入默认选择 `LEVEL 1`（C3–C5 自然单音）；另外两档是 C2–C6
自然单音与十二个 root 的常用原位大/小三和弦。Level 页旋转选择、短按开始；
题目页旋转不执行任何动作，短按才揭晓并播放。答案页默认聚焦 `NEXT`，旋转只在
`REPLAY / NEXT / LEVEL` 间移动焦点，短按才执行；因此旋钮误触不会丢题。首版不做
语音识别、评分、间隔复习、持久化或键盘位置图。
...
Normal interaction audio uses semantic `SoundCue` values only. SIGHT may also
submit a `MusicalNoteSet` containing one to four MIDI notes in piano range;
the platform still owns waveform, gain, duration, priority, voices, volume,
Replay restart, mute, and StopAll behavior. The authored cue palette lives
