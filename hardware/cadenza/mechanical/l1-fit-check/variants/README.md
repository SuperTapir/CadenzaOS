# L1 FPC 方向候选（历史探索，非当前放置权威）

> 当前 screen-only PCB 已固定 J_DISP1 于 KiCad `(154.625, 128.500)`。本目录旧候选只用于解释“屏幕局部方向”和“外壳旋转”是两个问题，不得用其中旧 FH12A/板边放置代替当前 FH34/USB fit-check。当前权威几何见上级目录 `generated/candidate-pcb-mechanical.json` 与 `l1-front-fitcheck-parameters.json`。
