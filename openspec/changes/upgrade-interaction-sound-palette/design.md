...
本次试听确定了 15 项 Semantic Hierarchy palette。用户同意正式实现不逐样本复制试听 WAV，只要合成结果在时长、低调程度、动作轮廓和上下文听感上没有明显偏离。Select、Turn On、Turn Off 只作为 Navigate、ToggleOn、ToggleOff 的本地校准参考，不进入仓库或产品；本地试听工作区 `09-semantic-hierarchy-full/confirm.wav` 与 `back.wav` 是 Confirm/Back 已拍板的原创方向原型，正式合成必须保留其双事件节奏、调音敲击材质、方向和时长边界，不能退回旧版连续滑音动机。

## Goals / Non-Goals

**Goals:**

- 把词汇扩展为 Input、Action、Outcome、System 四个家族共 15 项 cue。
