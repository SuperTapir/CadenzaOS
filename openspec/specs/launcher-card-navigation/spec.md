# Launcher Card Navigation Specification

## Purpose
定义 Launcher 多卡片连续轨道、循环选择、Cover 渲染、快速输入追赶、方向切换与无分配约束。
...
- **WHEN** Launcher 在任一支持 profile 下呈现 Timer、Motion、Settings 或 Gallery
...
- **WHEN** Timer 时间、Motion 位置或其他 App lifecycle 状态在运行期间发生变化后再次绘制其 Cover
